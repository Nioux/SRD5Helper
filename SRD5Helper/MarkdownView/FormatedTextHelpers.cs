...
namespace MarkdownView.MarkdownView
{
    public static class FormatedTextHelpers
    {
        ...
        public static void HtmlNodeToFormatedString(XmlNode node, FormattedString fs, FontAttributes attributes = FontAttributes.None)
        {
            if (node.NodeType == XmlNodeType.Text)
            {
                ...
                fs.Spans.Add(new Span() { FontFamily = fd.FontFamily, FontAttributes = attributes | fd.FontAttributes, FontSize = fd.FontSize, TextColor = fd.TextColor, Text = node.InnerText });
            }
        ...
        public class FontData
        {
            ...
            static T? ResolveOnPlatformToStruct<T>(string key) where T : struct
            {
                var resource = Application.Current.Resources[key];
                return resource as OnPlatform<T?>;
            }
            static T? ResolveSetterToStruct<T>(Setter setter) where T : struct
            {
                //if (setter?.Value is DynamicResource)
                //{
                //    var res = setter.Value as DynamicResource;
